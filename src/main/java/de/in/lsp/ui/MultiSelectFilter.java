...
/**
 * A filter component that displays a button and opens a popup with checkboxes for multi-selection.
 * 
...
public class MultiSelectFilter extends JPanel {
	...
	private void showPopup() {
		...
		selectAll.addActionListener(e -> {
			...
			// User requirement: "Select All as Reset". Select All selects EVERYTHING.
			if (selected) {
				selectedOptions.addAll(domainOptions);
			} else {
		...
		// Ensure popup doesn't close on click, similar to JMenu item handling but for custom component
	...
	private void rebuildPopupList() {
		...
		if (optionsToShow.isEmpty()) {
			...
			// Calculate proper height based on actual content We need to ask the contentPanel for its preferred size after adding
			// components
			...
			// Ensure min height for at least a few items if they exist but don't force it if content is small
