...
/**
 * A side/bottom panel that displays the full details of a single selected LogEntry.
 * 
...
public class LogDetailView extends JPanel {
	...
	public LogDetailView(Runnable onDetach, Runnable onClose) {
		...
		// CRITICAL: Force re-layout when width changes to allow WrapLayout to recalculate height based on new width
	...
	private void addField(String label, String value) {
		...
		TitledBorder titledBorder = BorderFactory.createTitledBorder(BorderFactory.createLineBorder(Color.LIGHT_GRAY), label);
		...
		valField.setBorder(BorderFactory.createCompoundBorder(titledBorder, BorderFactory.createEmptyBorder(0, 4, 2, 4)));
