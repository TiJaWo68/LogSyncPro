...
public class K8sPodSelectionFilterPanel extends AbstractTableFilterPanel<DefaultTableModel> {
    ...
    private void updateSelectAllState() {
        ...
        // Show "Checked" icon if not all are selected (suggesting "Select All")
        // Show "Unchecked" icon if all are selected (suggesting "Deselect All")
        selectAllCheckBox.setSelected(!allSelected);
    }

    /**
     * Extracts the human-readable base name of a pod by removing random suffixes.
     * e.g. "auditrepository-7fcd847b65-rkdlp" -> "auditrepository"
     * e.g. "act4telerad-0" -> "act4telerad"
     */
    private String getPodBaseName(String podName) {
        if (podName == null)
            return "";
        // Strip deployment hash + random suffix: -[8-10 chars]-[5 chars]
        // Strip statefulset/job index: -[digits]
    ...
    private void toggleSelectAll() {
        ...
        // If there's an unchecked row, clicking should check everything.
        // If all are checked, clicking should uncheck everything.
