...
/**
 * Handles remote-related actions like SSH connection and K8s discovery. Manages the connection workflow and log streaming from remote
 * containers.
 * 
...
public class RemoteActions {
	...
	private void showSshDialog(String host, String user, String password, String fetchFilter, LogViewListener listener,
			Map<Integer, Boolean> columnVisibility) {
		Composite panel = new Composite();
	...
	private void performSshDiscovery(String host, int port, String user, String password, String fetchFilter, LogViewListener listener,
			Map<Integer, Boolean> columnVisibility) {
		...
		new Thread(() -> {
			try (SshK8sService sshService = new SshK8sService()) {
				...
				for (var target : selected) {
					...
					LspLogger.info("Streaming logs from " + target.namespace + "/" + target.pod + "/" + target.container);
					try (InputStream is = sshService.streamLogs(target.namespace, target.pod, target.container)) {
						...
						SwingUtilities.invokeLater(() -> {
							viewManager.addLogView(entries, sourceName, columnVisibility, listener, ViewType.K8S);
						});
					} catch (Exception ex) {
	...
	private List<K8sPodSelectionDialog.SelectedContainer> autoSelectContainers(List<SshK8sService.K8sNamespace> namespaces, String filter) {
