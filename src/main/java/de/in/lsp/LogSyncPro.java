...
/**
 * Main application class for LogSyncPro. Manages the main window shell and delegates logic to specialized components.
 * 
...
public class LogSyncPro extends JFrame implements LogViewListener {
	...
	private static final String[] COLUMN_NAMES = { "Timestamp", "Level", "Thread", "Logger", "IP", "Message", "Source" };
	...
	private void setupInternalLogging() {
		internalLogView = viewManager.addLogView(new ArrayList<>(), "Internal Log", columnVisibility, this, ViewType.INTERNAL);
	...
	private void setupMenuBar() {
		...
		this.appMenu = new LogSyncProMenu(this, viewActions, remoteActions, helpActions, receiverManager, viewManager, columnVisibility,
				updateService, openLogsAction, exitAction);
		setJMenuBar(appMenu);
	}
	...
	private void backgroundLoadFiles(List<File> files) {
		fileActions.backgroundLoadFiles(files, status -> statusBar.setStatus(status, status.contains("Loading")), this, columnVisibility);
	}
