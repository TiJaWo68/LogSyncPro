...
public class UpdateService {
	...
	public UpdateService(JFrame owner, String currentVersion, UpdateChannel channel) {
		...
		this.updater = new GithubUpdater(this.currentVersion, new GithubReleaseSource("TiJaWo68", "LogSyncPro", configuredChannel), appJar);
	}
	...
	public void checkForUpdatesAsync(boolean manual) {
		CompletableFuture.runAsync(() -> {
			try {
				...
				if (info != null) {
					...
					if (manual) {
						SwingUtilities.invokeLater(
								() -> JOptionPane.showMessageDialog(owner, "Your version " + currentVersion + " is up to date.",
				...
				if (manual) {
					SwingUtilities.invokeLater(() -> JOptionPane.showMessageDialog(owner, "Failed to check for updates: " + e.getMessage(),
