...
public class SshK8sService implements AutoCloseable {
	...
	private String executeCommand(String command) throws IOException {
		try (ByteArrayOutputStream out = new ByteArrayOutputStream();
			...
			if ((!errorOutput.isEmpty() && out.size() == 0) && !errorOutput.toLowerCase().contains("password for")) {
	...
	private List<K8sNamespace> parseK8sOutput(String output) {
		...
		for (String line : lines) {
			...
			if (parts.length >= 2) {
				...
				// Handle both space-separated and comma-separated container names
				String[] containerArray = containersStr.split("[,\\s]+");
				List<String> containers = new ArrayList<>();
				for (String c : containerArray) {
					if (!c.trim().isEmpty()) {
